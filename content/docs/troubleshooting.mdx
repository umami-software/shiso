...
```text
notice: "integrations" is recognized but not implemented yet — it will be ignored.
```

This is expected: recognized keys that Shiso does not implement yet are accepted
and skipped so a shared config still builds. See
