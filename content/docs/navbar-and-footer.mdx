...
- Set `type` to `"github"` or `"discord"` to use its brand icon. Live star and
  member counts are not fetched.
- Set `icon` to use a name from the [lucide](https://lucide.dev/icons) icon set.
