...
- `navigation.versions`: multiple product versions — only the default version
  renders today; the others are reported and skipped
...
Groups also accept `hidden`, which hides the group and everything in it.

## Collapsible groups

Top-level groups render as static section headings. Nested groups are
collapsible: they start collapsed unless `expanded: true` is set or the
current page is inside them, and they reopen when you navigate into them.

The `interaction.drilldown` key controls what clicking a nested group header
does:
...
- unset: headers with a `root` page navigate; others just toggle
...
Anchors that contain their own `pages` or `groups` are part of the standard but
not implemented yet.
